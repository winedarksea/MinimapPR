...
- [x] **COP Track Symbology & Iconography** — tracks and detections on the COP map must be visually distinguishable by classification category (wildlife, human, vehicle, security, unknown) and by track status. Use simple iconography inspired by MIL-STD-2525C/APP-6 (pending, hostile, unknown, friendly shapes) consistent with the CoT type hierarchy defined in the Labels table. Phase 1 minimum: distinct icons or shapes per top-level label category, color-coded by track status (tentative/confirmed/coasting), and hostile/unknown/friendly fill. Must also include **velocity vectors (heading leaders)** and **uncertainty ellipses** (derived from Kalman covariance) to provide true situational awareness. A map of uniform dots is not a COP — category-distinguishable symbology with movement and uncertainty is what makes situational awareness possible at a glance.
...
- [ ] **Beamformed Classification** — feed classifiers with audio beamformed toward a localized source (MVDR or delay-and-sum) rather than raw omnidirectional audio, improving classification SNR in multi-source environments. The Phase 1 classifier interface is already source-agnostic; this item adds the beamforming stage between localization and classification. Depends on MVDR or delay-and-sum beamformer from Additional Localization Algorithms.
- [ ] **TAK / CoT Integration** — export tracks and detections as Cursor on Target events for interoperability with ATAK/WinTAK/iTAK.
- [ ] **MQTT Ingest Transport** — MQTT as an alternative *ingest* transport for node-to-server audio/sensor streaming alongside HTTP POST, natural for lightweight IoT/embedded nodes. Distinct from the Phase 1 MQTT *publish* client used for HA entity exposure — that client writes to HA's broker, this transport reads from nodes.
- [ ] **Sound Intensity Heatmaps** — aggregate pings and SPL data into spatial heatmap layers on the COP map.
- [ ] **Home Assistant Integration (Advanced)** — expand beyond Phase 1 alerting: expose active tracks as HA `device_tracker` entities for presence-based automation, fire custom HA events (`minimappr_detection`, `minimappr_track_update`) for automation triggers, map MinimapPR zones to HA areas for contextual rules, provide a minimal Lovelace dashboard card or panel iframe. Register HA **service calls** (`minimappr.arm_zone`, `minimappr.suppress_label`, `minimappr.cue_sensor`) as automation action primitives. Expose detection audio clips via HA's `media_source` platform for playback in the media browser and use in TTS/notification actions. Evaluate HA Add-on packaging (for Supervised/HAOS installs) and HACS custom integration packaging (for Core installs), including a config flow for guided setup.
- [ ] **Hybrid Array + Network Localization Fusion** — allow array nodes to publish local localizations while also contributing a network-level signal, then fuse both sources coherently via `source_type`.
...
- [ ] **COP Detection Audio Playback** — the ability to click a detection or track on the map and play the associated audio snippet. A sound-focused COP must allow the user to *hear* the evidence that triggered a detection directly from the map interface.
...
- [ ] **User Annotations** — frontend/API for annotating tracks and detections; feeds identity/fingerprint continuity, known-entity confidence, and training labels. Keep the front end clean and minimal — just enough to be effective. *(Moved from Phase 3 to align with Phasing & Prioritization plan; required dependency for Fingerprinting & Annotation Loop.)*
- [ ] **Authentication & Trust (Basic)** — API key or shared-secret authentication for node-to-server and server-to-server communication. Phase 1 operates unauthenticated (local/trusted network assumption); federation requires mutual authentication between peers. Design the auth layer to be upgradeable to TLS client certificates or token-based auth without changing the API shape. Also prerequisite for any internet-exposed deployment.
- [ ] **Sensor Cueing (Basic)** — webhook/notification mechanism triggered by rules engine to cue PTZ cameras, increase sample rates, or activate beamforming toward detections of interest. Initially simple HTTP webhook calls; complex effector tasking deferred to Phase 3. Scope is observation-side: directing sensors to *look at* a detection. Physical-response effectors (sprinklers, deterrents, strobes) are Phase 3. *(Moved from Unprioritized; architecture describes initial version as a simple mechanism.)*
- [ ] **Environment Table & Live Environmental Ingestion** — promote from schema stub to active table; ingest temperature/humidity/pressure from sensor-equipped nodes in real time to replace Phase 1's static speed-of-sound defaults. Directly improves core localization accuracy and is prerequisite for Phase 3 microclimate mapping. Records are tiny and long-retention. *(Moved from Phase 3 — localization fidelity depends on live environmental data, and the table is architecturally simple.)*
- [ ] **Track History Export & Replay Tooling** — read-only replay of the provenance chain (observation → detection → track update → track → alert) for algorithm tuning, forensic review, and ground-truth validation. Phase 1 persists the chain; this phase exposes it as a queryable API and optional timeline visualization. *(Carved from Unprioritized Track History & Replay — the storage side is Phase 1; the consumption side belongs here.)*
- [ ] **Moving Node Support** — continuous GPS location streaming from mobile nodes, dynamic location uncertainty weighting, `mobile` capability flag in node registry, IMU dead-reckoning gap-fill during GPS dropouts. Fusion engine weights mobile-node contributions by current location certainty.
- [ ] **Home Assistant MQTT/Webhook Integration** — implement the actual transport for the rules engine to send alerts to HA. Minimum viable: MQTT Auto-Discovery to expose detection-class binary sensors (e.g., `binary_sensor.minimappr_gunshot`, `binary_sensor.minimappr_bird`) and a per-zone activity sensor. Also support REST webhook fallback for HA installations without MQTT. This requires an MQTT *publishing* client (outbound to HA's broker), distinct from the Phase 2 MQTT *ingest* transport for node data.
- [ ] **Home Assistant Entity Exposure (Core Entities)** — register MinimapPR as an MQTT device in HA with: binary sensors for priority detection classes (gunshot, fire alarm, glass break, speech) using appropriate HA `device_class` values (`sound`, `safety`, `problem`), a numeric sensor for aggregate SPL per zone, and a diagnostic sensor for system health (node count, degraded nodes). Entities auto-expire via MQTT availability topics tied to heartbeat state. Also expose HA **event entities** (`event.minimappr_detection`) for transient detection events with structured `event_type` attributes — event entities are the modern HA pattern for impulse data that doesn't map well to binary sensor on/off state.
...
- [ ] **Ambisonic Spatial Encoder** — core rendering engine that takes localized, beamformed source audio and encodes it into First-Order Ambisonics (B-format W/X/Y/Z channels). Each localized source's bearing and distance map to ambisonics encoding coefficients. Input: per-source beamformed audio + position from the localization pipeline. Output: multi-channel B-format PCM. This is the shared foundation for both capture/export and live streaming. Depends on Phase 2 beamforming (MVDR or delay-and-sum) from Additional Localization Algorithms.
- [ ] **Soundscape Capture & Export** — select a time window (from stored observations or a live recording session), render a complete ambisonic soundscape file. Export formats: B-format PCM/WAV, IAMF/Eclipsa Audio (via [iamf-tools](https://github.com/AOMediaCodec/iamf-tools)), and 5.1 surround downmix for conventional playback. Capture sessions can be triggered manually, by schedule, or by rule (e.g., "capture 5 minutes of ambisonic audio whenever a rare bird species is confirmed"). Uses the offline/replay pipeline mode from Phase 1 for post-hoc rendering from stored observations. Spatial audio processing tooling: [SPARTA](https://leomccormack.github.io/sparta-site/) and [ambix](https://github.com/kronihias/ambix).
- [ ] **Privacy Filtering for Soundscape Output** — classification-conditioned source removal/suppression in the spatial render. When rendering a soundscape, sources classified as speech or vehicle (or any user-configured label set) are attenuated or excised before encoding. This enables privacy-safe public outputs (e.g., a nature reserve's bird soundscape without visitor conversations). Filtering applies equally to capture/export and live streaming modes. Depends on classification labels being attached to localized sources before spatial encoding.
...
- [ ] **STT (Speech-to-Text)** — transcription of human speech detections for conversational memory, incident detection, and sentiment mapping.
- [ ] **Passive Acoustic Mapping** — using ambient noise and echoes for blind 3D terrain reconstruction.
- [ ] **Self-Localization** — nodes using inter-node acoustic chirps to find and calibrate their own coordinates automatically.
- [ ] **Multi-modal Fusion** — integrating external data sources like RTL-SDR, ADS-B, camera, IMU/vibration for cross-domain awareness.
- [ ] **Effector Subsystem (Actions/Reactions)** — extends Phase 2's observation-side sensor cueing into physical-response effectors: smart sprinklers, strobes, deterrent speakers, directional audio projection, and automated perimeter responses. PTZ camera *slew-to-detection* is Phase 2 sensor cueing; PTZ camera *tracking sequences*, multi-effector coordinated responses, and threat-deterrence automations belong here. Rules engine conditions from Phase 1 serve as the trigger source. Must implement the full safety interlock model from the architecture: blue force lockout, zone-based arming, fire weather interlock, rate limiting, human-in-the-loop option, audit trail, and master arm switch.
- [ ] **Seismology / Infrasound** — ground vibration monitoring via IMU/accelerometer; low-frequency pressure wave detection.
- [ ] **Microclimate Mapping** — temperature/pressure/humidity trend mapping from distributed environmental sensors (depends on Phase 2 Environment Table & Live Ingestion).
...
- [ ] **Advanced COP Visualization** — extends the Phase 1 geographic COP map with: custom baselayer import (GeoTIFF, WMS, offline tile packages), track history trails with time scrubbing, acoustic coverage probability overlays, sound intensity heatmap layers from aggregated pings/SPL, interactive measurement tools (range/bearing between points), 3D altitude extrusion view, and multi-panel tiling for wide-area or federated deployments. Phase 1 establishes the geographic map with live tracks and health; this phase adds analytical and operational depth.
...
- [ ] **PostgreSQL / PostGIS Migration Path** — validate schema portability from SQLite to PostgreSQL with PostGIS for spatial queries at scale.
- [ ] **IMU noise filtering** - accelerometer/barometer subtraction for vibration artifact removal.
- [ ] **IFC Structure Import** - import IFC and other 3d models of structures
...
- [ ] **Detection Confidence Calibration** — tools for measuring and adjusting classifier confidence scores against ground-truth annotations to ensure thresholds in rules are meaningful across deployments and model versions.
