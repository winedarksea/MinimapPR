...
5. **Alerting** — simple rules engine with configurable conditions, routing to Home Assistant
6. **2D COP map** — tracks, detections, sensor health, GDOP overlay, category-distinct track symbology
...
- [ ] **Basic 2D COP Map** — minimal geographic map (Leaflet or MapLibre GL) displaying tracks, detections, and sensor health on real lat/long coordinates, replacing the current canvas-based prototype (Phase 1 item #6). Minimum: tile base layer, node markers, track icons with status color, detection markers with fade.
- [ ] **COP Map: GDOP Overlay** — render computed GDOP values (bare minimum) on the frontend map as a coverage-quality layer (Phase 1 item #6).
- [ ] **COP Track Symbology & Iconography** — tracks and detections on the COP map must be visually distinguishable by classification category (wildlife, human, vehicle, security, unknown) and by track status. Use simple iconography inspired by MIL-STD-2525C/APP-6 (pending, hostile, unknown, friendly shapes) consistent with the CoT type hierarchy defined in the Labels table. Phase 1 minimum: distinct icons or shapes per top-level label category, color-coded by track status (tentative/confirmed/coasting), and hostile/unknown/friendly fill. A map of uniform dots is not a COP — category-distinguishable symbology is what makes situational awareness possible at a glance.
- [ ] **WebSocket Event Filtering & Subscription** — allow COP clients to subscribe to events by zone, label category, minimum confidence, or track status via the `/ws/live` connection. Without filtering, active deployments will flood the COP with low-value updates. Initial implementation: client sends a JSON filter object after connection; server applies server-side filtering before broadcast. This also prepares for Phase 2 where multiple COP consumers (frontend, HA, TAK) need different event subsets from the same pipeline.
- [ ] **Zones & Exclusion Logic** — implement zones table (polygon geometry, zone types) and exclusion rules for suppressing expected classifications in defined areas, or alerting on classes only from defined areas.
- [ ] **Alerting & Rules Engine** — config-driven rules engine evaluating detections/tracks against user-defined conditions, routing alerts to Home Assistant and other destinations (Phase 1 item #5). Planned for shared structure with later effector functionality.
...
- [ ] **Classifier Chaining (Birdnet)** — YAMNet "bird" detection triggers BirdNET-Analyzer for species-level identification (Phase 1 item #3). This should not be a rigid component but built as a generalizable classifier chaining workflow (ie classifier fed from classifier, or classifiers from localized audio, or classifers from widefield non-localized audio).
...
- [ ] **Static Environmental Configuration** — configurable baseline temperature and humidity for speed-of-sound calculations, pending live sensor ingestion.
...
- [ ] **DCASE Alignment** — align classification taxonomy and evaluation methodology with [DCASE](https://dcase.community/) community; target integration with DCASE 2026 challenge best models.
- [ ] **Track History & Replay (Advanced)** — full interactive provenance replay with time-scrubbing, side-by-side algorithm comparison, and detection-level drill-down. The basic replay API and export tooling are in Phase 2; this covers the advanced visualization and analysis UI.
...
- [ ] **Snapcast / PipeWire Audio Distribution** — output validated to support synchronized multi-room playback of ambisonic or beamformed audio streams.
