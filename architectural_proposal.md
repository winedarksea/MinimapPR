...
- **Retention class** — `default`, `promoted`, `permanent` — controls cleanup behavior
...
6. **2D COP map** — tracks, detections, sensor health, GDOP overlay
...
- [ ] **MQTT Transport** — MQTT as an alternative ingest transport alongside WebSocket, natural for lightweight IoT/embedded nodes.
- [ ] **Sound Intensity Heatmaps** — aggregate pings and SPL data into spatial heatmap layers on the COP map.
- [ ] **Home Assistant Integration** — dedicated HA integration beyond alerting: entity exposure, automation triggers, dashboard panels.
...
- [ ] **Effector Subsystem** — programmatic triggers (Actions/Reactions) for PTZ cameras, smart sprinklers, strobes, and deterrent speakers.
- [ ] **Seismology / Infrasound** — ground vibration monitoring via IMU/accelerometer; low-frequency pressure wave detection.
- [ ] **Environment Table & Live Environmental Ingestion** — add long-retention environment records (temperature/humidity/pressure/lux) and real-time ingest to replace static speed-of-sound defaults.
- [ ] **Microclimate Mapping** — temperature/pressure/humidity trend mapping from distributed environmental sensors.
...
- [ ] **Sensor Cueing Framework** — webhook/notification mechanism to cue PTZ cameras, increase sample rates, or activate beamforming toward detections of interest.
- [ ] **Track History & Replay** — full provenance replay (observation → detection → track update → track → alert) for algorithm tuning and forensic review.
