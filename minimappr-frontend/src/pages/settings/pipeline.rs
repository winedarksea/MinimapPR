...
impl NodeEdit {
    fn from_node(node: &PipelineNode) -> Self {
        ...
        NodeEdit {
            mic_gains_db: node.mics.iter().map(|m| m.gain_db).collect(),
            hp_hz: first.map(|m| m.hp_hz).unwrap_or(0.0),
            lp_hz: first.map(|m| m.lp_hz).unwrap_or(0.0),
            smoothing: first.map(|m| m.smoothing.clone()).unwrap_or_else(|| "off".into()),
            dirty: false,
        }
    }
}
                ...
                <div class="mic-table mic-shared-row" style="margin-top:0.5rem">
                    <div class="mic-table-row" style="grid-template-columns:max-content 1fr 1fr 1fr">
                        ...
                        <input
                            ...
                            disabled=is_rust class="mic-input"
                        ...
                        />
                        <select disabled=is_rust class="mic-input" on:change=sm_chg>
                            <option value="off"
                    ...
                    view! {
                        <div class="pipeline-save-row">
                            <span class="muted" style="font-size:0.8rem">
                                "Settings stored — will apply on next Python-pipeline deployment."
                            </span>
                        </div>
                    }.into_any()
                }}
