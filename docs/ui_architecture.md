...
- Phase 6 slice: Added the first COP map context menu: MapLibre right-click now opens a workspace menu with copy-coordinates, center-map, disabled zone-draw placeholder, and guarded camera slew actions. Slew-to-map converts the clicked lat/lon through the configured site origin into local ENU meters before calling the existing effector aim endpoint; disarmed cameras and missing site origins show operator-visible disabled reasons.
