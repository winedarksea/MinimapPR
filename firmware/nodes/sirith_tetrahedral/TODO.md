...
```
```
conda activate minimappr && export MINIMAPPR_HOST=192.168.8.165 MINIMAPPR_PORT=8080 MINIMAPPR_INGEST_PORT=8081 MINIMAPPR_RUNTIME_PROFILE=birdnet_hybrid_production MINIMAPPR_INGEST_STORAGE_MODE=journal MINIMAPPR_DIRECT_INGEST_ENABLED=false MINIMAPPR_INGEST_SPOOL_READY_TTL_SECONDS=60 && minimappr
```
cleanup 
```bash
...
```
conda activate minimappr && export MINIMAPPR_HOST=192.168.8.165 MINIMAPPR_PORT=8080 MINIMAPPR_RUNTIME_PROFILE=birdnet_hybrid_production MINIMAPPR_DIRECT_INGEST_ENABLED=true MINIMAPPR_INGEST_PORT=8081 && unset MINIMAPPR_INGEST_STORAGE_MODE MINIMAPPR_INGEST_SPOOL_READY_TTL_SECONDS && minimappr
```
